...
class TradingViewSentimentAnalysis:
    ...
    def gpt_title_relevance(self, data, ticker):
        #title = data["Title"]
        title = data        
        ...
        response = self.client.chat.completions.create(
            ...
            messages = [
                {"role": "system",
                 "content": 'You are a financial analyst. Given an article title and a stock ticker, determine if the title of the article is relevant to the given stock. Your response should be either "Relevant" or "Discard" based on how much impact this article could have on the stock. Additionally, explain your reasoning behind your response in a concise manner.'
                },
                {"role": "user",
        ...
        relevance_decision = response['choices'][0]['message']['content'].strip()
        
        # Parse the response to extract the decision and reasoning
        if "relevant" in relevance_decision.lower():
            relevance = "Relevant"
        else:
            relevance = "Discard"
        
        # Return both the decision and the explanation
        explanation = relevance_decision.replace(relevance, "").strip()
    ...
    def gpt_article_sentiment(self, text, ticker):
        response = self.client.chat.completions.create(
            ...
            messages = [
                {"role": "system",
                 "content": 'You are a financial analyst. You will be given the text of an article and a stock ticker. Analyze the sentiment with respect to the given ticker You response should be one of the following: "Positve", "Negative", or "Neutral" based off the overall sentiment of the article. Additionally, provide a brief explanation of your reasoning.'
                },
                {"role": "user",
        ...
        # Parse the response to extract the sentiment and explanation
        ...
        explanation = sentiment_analysis.replace(sentiment_label, "").strip()
        return {"Sentiment": sentiment_label, "Explanation": explanation}
    ...
    def test(self):
        ...
        print(f"GPT Title Sentiment: {gpt_title_sent}")
        print(f"GPT Text Sentiment: {gpt_text_sent}")
        print(f"FinBERT Text Sentiment: {finbert_text_sent}")
